...
class NordcapBaseCrawler:
    ...
    def __get_page(self, base_url, page_number):
        ...
        return self._get_soup(page_url)

    def __get_total_pages(self, page):
        indicator = page.find_all("span", "paging--display")[0]
        number = int(indicator.find_all("strong")[0].text)
        return(number)

    def __get_pages(self, base_url):
        pages = []
        first_page = self.__get_page(base_url, 1)
        pages.append(first_page)
        total_pages = self.__get_total_pages(first_page)
        for page_number in range(2, total_pages + 1):
            pages.append(self.__get_page(base_url, page_number))
        return(pages)

    def __get_product_urls(self, base_url):
        product_urls = []
        pages = self.__get_pages(base_url)
        for page in pages:
    ...
    def run(self):
        ...
        for base_url in self.urls:
            product_urls = product_urls + self.__get_product_urls(base_url)
